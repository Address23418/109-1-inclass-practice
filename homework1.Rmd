...
考量第四戰熱火輸球，湖人聽牌，原本想等到第七戰才夠刺激，三人決定提前一起收看第五場比賽，請將三人準備要一起觀賽的表定賽程資訊(包含日期`Date`與開賽時間`StartTime`, 還有`TimeZone`）從List裡面抽出來，另存為一個atomic vector，叫做`Game5 Time`。
...
```{r ans13}
# G5TimeParsed
Game5. <- c("Game5","Oct.9","2020","21:00")
Game5.[[2]]
Game5.[[3]]
Game5.[[4]]
G5TimeParsed <- paste0(Game5.[[2]],Game5.[[3]],Game5.[[4]])  
```
...
```{r ans14}
# TpeJason  Game 5 : Friday, Oct. 9: Miami Heat at L.A. Lakers, 21:00 ET
library(lubridate)
AllenTime <-  ymd_hms("2020-10-09 21:00:00",
        tz="America/New_York")
...
```{r ans15}
# ParisLouis
AllenTime2 <- ymd_hms("2020-10-09 21:15:00",
        tz="America/New_York")

ParisLouis <- with_tz (AllenTime2,tzone = "Europe/Paris")
...
```{r ans22}
# sumChNumbersWith0
數字3 <- factor(chNumbersWith0)
數字4 <- as.numeric(數字3)
sumChNumberwith0 <- sum(數字4)
