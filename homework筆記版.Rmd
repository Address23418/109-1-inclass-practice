...
Game5Time1 <- c(Game5$Date,
                   Game5$StartTime,
                   Game5$TimeZone )

cat(Game5Time)#'cat' 目前還不能用 1 引數 (類型 'list')，cat不能用list
cat(Game5Time1)
...
G5TimeParsed <- paste0(Game5Time[[1]]," 2020 ",Game5Time[[2]])#少了時間、時區的表示

#G5TimeParsed1 <-  mdy_hm(paste0(Game5Time[[1]], " 2020 ", Game5Time[[2]]),tz="America/New_York")
...
```{r ans14}
# TpeJason  "Oct.9 2020 21:00"
AllenTime <- mdy_hms ( "Oct.09,2020,21:00:00", tz="America/New_York")

TpeJason <- with_tz(AllenTime,tz="Asia/Taipei") #少了時間文字寫法
...
```{r ans15}
#ParisLouis
LouisBackTime <- mdy_hms("Oct.09,2020,21:15:00",tz="America/New_York")

ParisLouis <- with_tz(LouisBackTime,tz="Europe/Paris")

#ParisLouis <- with_tz(LouisBackTime,tz="Europe/Paris")+minutes(15)---minutes的用法

#with_tz(NewYorkAllen, tzone = "Europe/Paris") + minutes(15) -> ParisLouis
```
...
```{r ans21}
# sumChNumbers
A <- factor(chNumbers) #少了levels，電腦的大小排序會出問題/factor的意義是把字串轉成class factor
...
C <- factor(chNumbersWith0) #少了levels，電腦的大小排序會出問題

D <- as.numeric(C)

sumChNumbersWith0 <- sum(D) #level的第一個自會記成1，意思是所有排序都+1，因此最後算總和要剪掉長度

#fct_chNumbersWith0 <-  factor( chNumbersWith0,levels = ch0to9 )
#sumChNumbersWith0 <- sum(as.integer(fct_chNumbers)) - length(chNumbersWith0)
...
```{r ans32}
# dearStudent
sen1<- student$deadline
    
sen2<- with_tz(sen1,tzone ="UTC")

TimeUTC <- as.character(sen2)

sen3 <- paste0("Dear",student$name,":","\n貼心提醒您，作業上傳截止時間是",
               TimeUTC,"\n敬祝學安","\n你的貼心學伴巴西利bot silly") #時間文字表示錯
...
```{r ans4}
download.file(url="https://www.dropbox.com/s/rykdkmz27o1t4dh/IMG_2027.JPG?dl=1",destfile="hw1.jpg")
library(magick) # 處理圖片的套件

location1 = download.file(url="https://www.dropbox.com/s/rykdkmz27o1t4dh/IMG_2027.JPG?dl=1",destfile="hw1.jpg")

save(location1,file ="hw1.jpg" )
...
save(location1,file = meta)  #建立缺少的package
